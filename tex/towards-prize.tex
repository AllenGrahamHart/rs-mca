...
The following consequences of the cap paper \cite{Cho26b} may be combined with \cref{thm:main}.
\begin{enumerate}
...
\item \textup{(Finite staircase certificates.)} A row certificate consists of an unsafe prefix/rational-floor handle and a safe handle \textup(deep, half-Johnson, or the single imported half-distance CA theorem\textup).  Once its integer inequalities are checked, it proves a concrete interval
\[
        \frac{r_{\rm safe}}n
        \le
        \delta^*_C(\varepsilon^*)
...
\]
Adjacent safe/unsafe certificates determine the integer staircase exactly.  In particular, for the KoalaBear sextic row at target $2^{-128}$,
\[
\begin{array}{ll}
        \delta^*_C\in
...
\end{array}
\]
For the Mersenne-$31$ circle line-round row at target $2^{-100}$, the corresponding intervals are
\[
\begin{array}{ll}
        \delta^*_C\in
...
Clause \textup{(1)} is the elementary half-Johnson CA theorem of \cite{Cho26b}: two bad slopes recover a pair-list at agreement $n-2r$, the pair-interleaved Johnson bound gives $L_2(r)$, and the pincer from CA to MCA transfers the result below half the distance. Clause \textup{(2)} is the finite scanner/certificate closure of \cite{Cho26b}: unsafe handles are prefix or rational floors composed with the deep-point conversion, while safe handles are the deep, half-Johnson, or imported half-distance certificates. The displayed intervals are the exact integer certificates printed there. Clause \textup{(3)} is the optimized form of the same deep-point conversion: rearranging its dichotomy with $\eta=\eca/T$ gives the factor $\kappa/(\kappa+1)$.
\end{proof}
