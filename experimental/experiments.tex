...
The width-one critical-tail branch has a compact fixed-root reduction.  At a
base-free canonical \(b=2\) node, let
\[
...
\]
Equivalently, width-one has a bounded-complement rank test: for
\(O\subset D^{can}\), \(|O|=s+1\), put
\[
  L_O=\ell_{D^{can}\setminus O}.
\]
Then \(O\) supports a width-one certificate if and only if
\[
  L_O\in V,
\]
or equivalently all \(3\times3\) coefficient minors of \(P,Q,L_O\) vanish.  In
fixed surplus the number of such tests over the active tree is at most
\[
  (s_0+2)Q^{s_0+1}\binom{Q+s_0}{s_0+1},
\]
where \(Q\) is the initial quotient width.
For any independent \(B\in V\), base-freeness gives \(B(x)\ne0\) on \(Z\), so
the projective map is constant on the large fiber:
\[
  [A(x):B(x)]=[0:1]\quad(x\in Z).
\]
All nonconstant projective information is confined to \(O\), and the good-pair
capacity splits as
\[
  G^{can}=(q-1)(s+1)+G_O,\qquad 0\le G_O\le\binom{s+1}{2}.
\]

This large cube descends losslessly.  If \(P_m\subset Z\), then
\[
  A^{P_m}=A/\ell_{P_m}=c\ell_{Z\setminus P_m},\qquad
  D^{can}\setminus P_m=(Z\setminus P_m)\sqcup O,
\]
so the descended width is still one and the bounded complement is unchanged.
With \(a=\lfloor(q-2)/2\rfloor\), the co-small flag cube has the first-root
partition
\[
...
\]
where the right side is empty when \(a=0\).  After exposing the first root
\(x\), the divided direction \(A^x=A/(X-x)\) has parameters
\[
  q^x=q-1,\qquad r^x=1,\qquad s^x=s,\qquad O^x=O,
\]
so every nonempty width-one flag injects into a one-root absorbed
fixed-divisor/root-slice family.
This injection is also quantitatively localized.  If
\[
...
\]
The last inequality uses that \(a=\lfloor(q-2)/2\rfloor\) is central or one
step below central.  Hence any large width-one cube already has a large
single-root absorbed slice; the obstruction cannot be hidden as a
many-first-root averaging effect.
Equivalently, if all such first-root slices satisfy \(M_x\le C Q^K\), then
\[
  q\le K\log_2 Q+2\log_2 q+\log_2(2C)+1.
\]
Thus polynomial control of the one-root fixed-divisor slices forces every
uncharged width-one certificate into logarithmic quotient width; any
large-width survivor must already be a large one-root slice counterexample.

Consequently, in fixed surplus \(s_0\le\sigma\), small nodes \(q_S\le s_S+2\)
are polynomial by the active-tree bound, while on \(q_S>s_S+2\) there is at
most one width-one shadow per node.  Thus
\[
  WO_1(A_0)
  \le FixedRootOneRoot_{r1}(A_0)+O_\sigma(Q^{\sigma+1}),
\]
where \(Q\) is the initial quotient width.  A polynomial fixed-surplus bound
for the displayed one-root fixed-root/root-slice ledger closes the width-one
critical-tail branch; this is still a local closure criterion, not the final
all-line M1 theorem.
