...
- **Agent/model:** Codex acting autonomously for AllenGrahamHart.
- **Files added or changed:**
  `experimental/scripts/verify_f17_32_m3_rank6_a385_no_fixed_core_pressure.py`;
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED / AUDIT.
- **What is being added:** A necessary profile for any separated `A=385`
  no-fixed-core over-budget survivor: it must have six finite classes plus an
  unpaid endpoint, at least `738` external-root incidences, and some pair of
  finite classes sharing at least `24` external roots.
- **How it is useful:** This converts the remaining no-fixed-core/moving-core
  frontier into a concrete large pair-core target for quotient, product
  collapse, or Hankel-specific exclusion.
- **What to do next:** Attack the large pair-core branch: determine whether a
  `24`-root external pair core is quotient-paid, forces product collapse, or
  contradicts separated low-degree Hankel transfer.
...
- **Agent/model:** Codex acting autonomously for AllenGrahamHart.
- **Files added or changed:**
  `experimental/scripts/verify_f17_32_m3_rank6_a385_two_core_high_core_closure.py`;
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED / AUDIT.
- **What is being added:** A closure of the separated `A=385` rank-6 fixed
  two-core high-core line/conic residual: line components close for
  `71<=e_G<=122` by product collapse, and the punctured tangent tail is
  projective-budget safe for `e_G>=122`.
- **How it is useful:** Together with the incidence and conic product-collapse
  packets, this makes every separated fixed two-core line/conic moving-slope
  component projective-budget safe.
- **What to do next:** Move outward from the fixed two-core hypothesis:
  moving-core/no-common-core branches, overlapping-support rank-6 pencils, and
  row-level M3 synthesis remain separate targets.
...
- **What is being added:** The `A=386` moving-slope packet now has a
  ...
  and a Pascal obstruction profile for the extremal conic `e_G=69` branch.
- **How it is useful:** A seven-slope saturated tangent-star tail has at most
  ...
  four residual coordinates have conic-evaluation determinant
  `prod_{i<j}(x_j-x_i)^2`.  The
  ...
  line rows `e_G=72..80` must contain six fully split quotient-pencil fibers
  of degrees `54..46`, and the conic rows `e_G=69..76` must contain six fully
...
- **Files added or changed:**
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED / AUDIT.
- **What is being added:** The `A=386` moving-slope packet now records a compact
  exact design catalog for the whole endpoint-only finite-incidence one-over
  range, not only the first line/conic rows.
- **How it is useful:** Line cores `e_G=72..80` have allowed histogram counts
  `2,16,27,28^6`; conic cores `e_G=69..76` have counts `2,16,27,28^5`.
  This shows precisely where base splitting and pair-overlap pressure disappear
  and where later arguments must use a different saving.
- **What to do next:** Use the catalog to split future closure attempts: base
  collision arguments for the first few cores, and quotient/tangent/external
  slack arguments once all base histograms are possible.
...
- **Files added or changed:**
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED / AUDIT.
- **What is being added:** The `A=386` moving-slope packet now enumerates exact
  degree-`126` root-budget alternatives for the extremal line `e_G=72` and
  conic `e_G=69` survival shapes.
- **How it is useful:** Line histogram `(0,0,6)` leaves exactly one unused
  nonforced external root line, while `(0,1,5)` uses them all.  Conic histogram
  `(0,0,6)` needs `14` pair-overlaps, while `(0,1,5)` needs all `15`.
- **What to do next:** Attack these exact alternatives directly by proving the
  all-used/all-paired configurations are impossible, paid, or force duplicate
  slopes.
...
- **Files added or changed:**
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED / AUDIT.
- **What is being added:** The `A=386` moving-slope packet now records
  necessary saturation conditions for the endpoint-only one-over-budget
  subranges.
- **How it is useful:** Six finite line classes in the incidence one-over range
  must have pairwise disjoint external root sets with slack `1..41`; six finite
  conic classes need `0..14` forced pair-overlap events before external excess;
  the `e_G=120` cases must saturate the punctured projective tangent bound.
- **What to do next:** Use these saturation targets to either exclude the
  six-class patterns, identify them as paid quotient/tangent structure, or find
  a minimal witness showing they genuinely survive.
