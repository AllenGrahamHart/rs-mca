...
import verify_x81_minimal_trade_square_shift as x81
...
def forced_root_and_obstructions(locator: list[int], h: int, p: int) -> dict[str, Any]:
    ok, root, lambda_const = x81.forced_square_shift(locator, h, p)
    square = x81.poly_square(root, p)
    ...
    return {
        ...
        "nonzero_square_lambda": lambda_const != 0 and x78.is_square_mod(lambda_const, p),
    }
...
def algebra_checks() -> list[dict[str, Any]]:
    ...
    inv4 = pow(4, -1, p)
    for h in range(3, 9):
        a_poly = x81.locator_from_roots(list(range(2, 2 + h)), p)
        delta = (2 * h + 3) % p
        b_poly = list(a_poly)
        b_poly[0] = (b_poly[0] + delta) % p
        union_locator = x81.poly_mul(a_poly, b_poly, p)
        ...
        check(
            ...
            poly_add_const(union_locator, data["lambda"], p) == x81.poly_square(data["root"], p),
        )
...
def row_report(n: int, p: int, h: int) -> dict[str, Any]:
    ...
    for subset in combinations(range(n), h):
        locator = x81.locator_from_exponents(domain, subset, p)
    ...
    for support in combinations(range(n), 2 * h):
        support_count += 1
        locator = x81.locator_from_exponents(domain, support, p)
