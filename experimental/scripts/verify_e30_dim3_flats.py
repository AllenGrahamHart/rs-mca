...
CERT = os.path.join(
    ...
    "data",
    "certificates",
    "e30-dim3-flat-census",
    "e30_dim3_flat_census.json",
)
...
def main() -> int:
    ...
    ap.add_argument("--write-certificate", action="store_true",
                    help="write the JSON certificate for the full deterministic run")
    args = ap.parse_args()
    ...
    cert = {
        "task": "E30 dimension-3 flat census",
        "node": "rk_axis_d_dim3_flat_census",
        "status": "EXPERIMENTAL / toy evidence; sampled census, not exhaustive and not a proof",
        "scope": "F_17, mu_16, projective-dimension-3 flats; j=3 exact, j=4,5 deterministic sampled families",
        "families": emitted,
        "family_count": len(emitted),
        "all_expected_families_pass": all_pass,
    }
    ...
    if os.path.exists(CERT):
        with open(CERT, encoding="utf-8") as fh:
            expected_cert = json.load(fh)
    cert_exists = expected_cert is not None
    cert_match = cert_exists and cert == expected_cert
    print(f"certificate exists: {'PASS' if cert_exists else 'FAIL'}")
    print(f"certificate matches recomputed summary: {'PASS' if cert_match else 'FAIL'}")
    all_pass &= cert_exists and cert_match
