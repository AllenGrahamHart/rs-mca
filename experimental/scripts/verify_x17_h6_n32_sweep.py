#!/usr/bin/env python3
"""X17 h=6/t=5 exact signature sweep for the terminal active-core node.

This verifier extends the signature-injectivity evidence beyond X15.  It scans
all 6-subsets of mu_32 at the standard alpha rows and groups them by the top
five elementary-symmetric sums.  Signature injectivity means no h=6 active
partner exists, even before applying any paid strip.
"""
...
def code6(comb: tuple[int, int, int, int, int, int]) -> int:
    out = 0
...
def exps6(code: int) -> list[int]:
    return [(code >> (8 * shift)) & 255 for shift in range(6)]


def signature_arrays(
    p: int, n: int, domain: list[int]
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Return h=6/top-5 signature arrays sorted by the three key words."""
    h = 6
    t = 5
    max_pair_key = p**2 - 1
    check(
        f"n={n}, p={p}: paired h=6 signature words fit uint64",
        max_pair_key < 2**64,
        f"p^2={p**2}",
    )
    ...
    for comb in combinations(range(n), h):
        ...
        key_5.append(e[5])
        codes.append(code6(comb))

    key_12_arr = np.frombuffer(key_12, dtype=np.uint64).copy()
    key_34_arr = np.frombuffer(key_34, dtype=np.uint64).copy()
    key_5_arr = np.frombuffer(key_5, dtype=np.uint64).copy()
    code_arr = np.frombuffer(codes, dtype=np.uint64).copy()
    order = np.lexsort((key_12_arr, key_34_arr, key_5_arr))
    return key_12_arr, key_34_arr, key_5_arr, code_arr, order
...
def analyze_row(row: SweepRow) -> dict[str, Any]:
    ...
    key_12_arr, key_34_arr, key_5_arr, code_arr, order = signature_arrays(
        p, row.n, domain
    )
                ...
                collision_examples.append(
                    {
                        ...
                        "signature_5": int(k5),
                        "subsets": [exps6(int(code_arr[order[i]])) for i in range(start, end)],
                    }
                )
        start = end

    check(
        f"{label}: no h=6 top-five signature collisions",
        collision_groups == 0,
    ...
    return {
        ...
        "subset_count": math.comb(row.n, 6),
...
def build_certificate() -> dict[str, Any]:
    ...
    check("all checked h=6 rows have injective top-five signatures", all_injective)
    check(
        "sweep includes n=32 through alpha=3",
        any(row["label"] == "n32_alpha_3" for row in rows),
    )
    check(
        "h=6 n=32 rows stay below one million subsets per row",
        max(row["subset_count"] for row in rows) < 1_000_000,
    )

    return {
        "task": "X17 h=6 n=32 signature sweep",
        "node": "active_core_count_bound",
        "status": (
            "EXACT FINITE EVIDENCE: h=6 has no active partners before stripping "
            "in every checked n=32 row"
        ),
        "scope": (
            "h=6/t=5 split pairs at n=32; since the top-five signature is "
            "injective on all checked 6-subsets, no active-core partners exist"
        ),
        "coverage_note": (
            "n=32 is checked through alpha=3; n=64,h=6 would require sorting "
            "C(64,6)=74974368 subsets and is intentionally not part of this "
            "low-memory packet"
        ),
        "signature_encoding": "(e1+p*e2, e3+p*e4, e5)",
        "rows": rows,
        "summary": {
...
def main() -> int:
    ...
    print(f"\nPASS: {NCHECK} X17 h=6 n=32 signature-sweep checks")
    return 0
