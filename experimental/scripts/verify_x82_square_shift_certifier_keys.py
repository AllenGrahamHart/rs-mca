...
def square_shift_data(domain: list[int], support: tuple[int, ...], p: int, h: int) -> dict[str, Any]:
    ...
    square_lambda = ok and lambda_const != 0 and x78.is_square_mod(lambda_const, p)
    return {
        "ok": ok,
        "lambda": lambda_const,
...
def algebra_checks() -> dict[str, Any]:
    ...
    locator = x78.locator_from_roots(roots, p)
    ok, root, lambda_const = x81.forced_square_shift(locator, h, p)
    scaled_roots = [(scale * value) % p for value in roots]
    scaled_locator = x78.locator_from_roots(scaled_roots, p)
    scaled_ok, scaled_root, scaled_lambda = x81.forced_square_shift(scaled_locator, h, p)
