"""Offline replay of the spectral-mask and actual shared-core extension."""
...
MANIFEST_HASH = "693c0120e5d2b9e9a868dc250af8f893ec0e234ccfc5f93c7f99f6742aecb5dd"
PARENT = "918324a10b1d8235e5d06418cbfdca83544d339c"
PRIOR = "kb-regular-terminal-actual-span-20260910"
HELPER_HASH = "31e822f5063e30e59d4c709f9f0298431207090582751a71d6e55574c746dd2e"
ROOTS = ["pair_space_operator_spectral_fibre_johnson",
         "rate_half_mca_regular_terminal_rich_fibre_frontier"]
SCOPE = {
    "lane": "K3 / DIRECT", "object": "MCA", "agreement": 1116048,
    "field": "2130706433^6", "original_error_rank": 12,
    "shared_carrier_dimension": 11, "J": [9965, 21499],
    ...
    "root_mask": "anchors and F-eigenpolynomial roots; common carrier zeros when e=0",
    "intersection_scope": "two actual complete joint cores outside the mask",
    "spectral_counts": [0, 1, 2, 3], "uniform_intersection_caps": [1913, 1913, 1022, 169],
    "profiles": 13, "spectral_gates": 52, "cutoff_tests": 104,
    "actual_core_witness": True, "original_evaluation_flat_rank": 9,
    "generic_degree_used": False, "joint_evaluation_rank_two_required": False,
    ...
    "all_terminals_covered": False, "rank19_closed": False,
...
def load_prior():
    ...
    spec = importlib.util.spec_from_file_location("frozen_terminal_custody", path)
    prior = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(prior)
    raw = prior.checked_file(prior.ROOT, "SOURCE_MANIFEST.json")
    need(digest(raw) == prior.MANIFEST_HASH, "parent manifest pin")
    manifest = json.loads(raw)
    published, graph = prior.suppliers()
    parent_sources = prior.validate(manifest, published, graph)
    published.update({(PRIOR, path): data for path, data in parent_sources.items()})
    graph.update(manifest["requirements"])
    need(len(published) == 1194, "inherited source inventory")
    return prior, published, graph


def validate(manifest, prior, published, credited):
    need(manifest["schema"] == "kb-regular-terminal-rich-core-fibres-v1", "schema")
    need(manifest["parent_commit"] == PARENT and manifest["snapshot_roots"] == ROOTS,
         "snapshot identity")
    need(manifest["supplier_manifests"] == dict(prior.SUPPLIERS, **{PRIOR: prior.MANIFEST_HASH}),
         "supplier pins")
    ...
    need(manifest["inherited_proof_documents_compared"] == 337
         and manifest["published_source_hashes_checked"] == 1194, "inherited counts")
    files = {row["path"]: row for row in manifest["files"]}
    need(len(files) == len(manifest["files"]) == 22, "new file inventory")
    ...
    need(sum(map(len, sources.values())) == manifest["source_bytes"] == 95671,
...
def mutations(manifest, prior, published, graph):
    cases = []
    ...
    for bad in cases:
        try:
            validate(bad, prior, published, graph)
...
def main():
    prior, published, graph = load_prior()
    raw = prior.checked_file(ROOT, "SOURCE_MANIFEST.json")
    need(digest(raw) == MANIFEST_HASH, "manifest pin")
    manifest = json.loads(raw)
    sources = validate(manifest, prior, published, graph)
    print("PASS 1216 listed hashes; 337 inherited documents; 112-node acyclic closure;",
          mutations(manifest, prior, published, graph), "malformed manifests rejected", flush=True)
    borrowed = {}
    ...
    checks = [(ROOTS[0], "verify.py"), (ROOTS[1], "verify.py"),
              (ROOTS[1], "verify_audit.py"), (terminal, "verify_audit.py")]
    env = dict(os.environ, PYTHONDONTWRITEBYTECODE="1")
    with tempfile.TemporaryDirectory(prefix="kb-rich-core-") as temp:
        runtime = Path(temp)
    ...
    print("PASS three new checks and the inherited rational-plane independent audit")
    print("Other inherited arithmetic not freshly replayed; independent mathematical review due")
    print("Rich-core mass, remaining terminal classes and both Prize problems remain open")
