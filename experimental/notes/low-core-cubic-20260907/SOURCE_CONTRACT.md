...
The interval theorem proves the bound for EVERY source in this contract
with 4801<=J<=7116. It assumes no interpolation relation, conic cover,
field-drop, random-fiber assertion, or numerical MCA Hensel bound.
...
The first line requires the declared single-conic premise. The second
does not. Neither is an unrestricted original-row theorem.
