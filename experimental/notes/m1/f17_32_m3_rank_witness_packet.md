...
have no common component, Bezout gives at most four compatible `Q`-classes,
hence at most four finite noncontained slopes; adding the endpoint gives total
projective contribution `<=5`.  The remaining fixed two-core residual is the
common-component branch on that `Q`-plane.

The `A=385` two-core component-cut companion refines that residual.  If every
irreducible component of a common component is cut by some
direction-consistency conic, the component plus off-component Bezout count
gives at most four finite `Q`-classes and total projective contribution `<=5`.
...
The `A=385` fixed-core synthesis companion packages the fixed-core tree.  It
verifies that every separated rank-6 branch with a fixed forced base core of
size at least two is projective-budget safe.  Consequently the remaining
separated `A=385` obstruction must avoid a fixed two-point base core; moving-core
