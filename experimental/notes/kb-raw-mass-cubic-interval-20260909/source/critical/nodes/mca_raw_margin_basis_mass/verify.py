...
def main():
    ...
    for cutoff in range(1, 7):
        for kappa in range(1, cutoff+2):
            for beta in range(1, 5):
                high = kappa*beta
                for raw in range(1, 12):
                    cost = raw*beta if raw <= cutoff else high
                    need(cost >= min(raw, kappa)*beta, "separately funded high-record variant")
                    high_cases += 1
                need(high-1 < kappa*beta, "underfunded high-record control")
    print("PASS three actual multi-defect sources,", count, "flag cases and", owner_cases, "owner cases")
    print("PASS", high_cases, "generalized HIGH integer controls; the actual high-record bound remains an input")
