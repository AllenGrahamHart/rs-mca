...
For the stated generalization, substitute the separately proved H in
this paragraph. The LOW insertion argument is unchanged, and raw>T
implies raw>=kappa. Thus H>=kappa*beta is exactly the high-record charge
needed. No other step uses the baseline formula for H.
