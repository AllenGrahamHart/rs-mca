...
def verify(residual=(1027077, 1038611), total=274979661975561635):
    ...
    paid = [(0, 793576, 273540953998915577),
            ...
            (1038612, 1043775, 274979661975561635),
    ...
    check((K-residual[1], K-residual[0]) == (9965, 21499), "unpaid child interval")
    check((K-1027076, K-878577) == (21500, 169999), "whole-interval child transport")
    check((K-1043775, K-1038612) == (4801, 9964), "lower child transport")
    check(residual[1]-residual[0]+1 == 11535, "residual cardinality")
    check(12559-11559 == 22499-21500+1 == 1000, "new whole-source degree interval")
    check(11559-11535 == 9964-9941+1 == 24, "new complete cubic interval")
...
def main():
    verify()
    mutations = [((1027076, 1038611), 274979661975561635),
                 ((1027078, 1038611), 274979661975561635),
                 ((1027077, 1038610), 274979661975561635),
                 ((1027077, 1038612), 274979661975561635),
                 ((1027077, 1038611), 274979661975561634),
                 ((1027077, 1038611), 274979661975561636)]
    ...
    verify_raw_mass_cubic_interval()
    print("PASS: exact original-g partition, child intervals, one near and field reserve")
    print("PASS: six boundary/budget mutations rejected; original rank-twelve residual J=9965..21499")
...
def verify_two_cost_interval():
    ...
    check(max(9965,23000)>min(21499,29999), "old density/mass scopes miss entire residual")
...
def verify_clustered_arc_gate():
    ...
    check((max(9965,20481),min(21499,22999))==(20481,21499), "remaining useful source-class range")
...
def verify_raw_mass_cubic_interval():
    for J, expected in ((9940, False), (9941, True), (9964, True), (9965, False)):
        check((9941 <= J <= 9964) == expected, "complete cubic interval endpoints")
    K, total = 1048576, 274861473951141154
    check((K-9964, K-9941) == (1038612, 1038635), "24 complete original core sizes")
    check(2130706433**6//2**128-total == 119254160253933, "new source reserve")
    check(total < 274979661975561635, "unchanged main lower-union maximum")
    check(11559-11535 == 9964-9941+1 == 24, "current exact residual reduction")
    print("PASS: raw-mass cubic interval removes24 whole degrees, no source-cover premise")
