...
LOW,HIGH = 23000,23999
TOTAL = 274846585959022192
EXPECTED = "a71e75b1e32f0ec5715bc7e37b97d903e3cbd4b01543d21f661b9789e94e9a93"
...
def profile_bound(rank,k0,k1,hn,hd):
    check(D>=1 and k1>=k0>=rank>=2 and hn>=hd>0,"whole profile box")
    coefficients = {}
    for r in range(3,rank+1):
        j = rank-r
        root = (k1-rank+1,D+k1-rank+r)
        local = ((j+1)*hn-j*hd,hd*(D+k0-j))
        small = root if root[0]*local[1]<=local[0]*root[1] else local
        n,d = maximum((1,r-1),small)
        check(0<n<d,"strictly positive contraction")
        coefficients[r] = (d-n,d)
    numerator,denominator = D+1,1
    for a in range(2,rank+1):
        an = prod(coefficients[r][0] for r in range(a+1,rank+1))
        ad = prod(coefficients[r][1] for r in range(a+1,rank+1))
        numerator *= (D+1)*ad+(k0-1)*an
        denominator *= ad
...
def cost(left,right,t,k0,k1):
    ...
    n,d = maximum((qn*inner,qd),(hybrid,1),profile_bound(11,left,right,last,t))
    return reduce_pair(12*n,d)


def main():
    digest = hashlib.sha256()
    peak,blocks,count,mutations = 265879110627611677,0,0,0
    left = LOW
    while left <= HIGH:
        right = min(left+15,HIGH)
        resource = prod(range(R+right-11,R+right+1))
        ln,ld = profile_bound(11,left,right,right-4,7)
        ...
        for t in range(1,7):
            ell = 11-t
            f = ell*ell//4
            start = 8001 if t==1 else ell
            end = right-t*(right-4)//7-1
            extra = [(D+f*(ell-1))//(f-1)+1]
            for k0,k1 in old.partition(start,end,128,extra):
    ...
    check(blocks==63 and count==48357 and mutations==96714,"entire independent inventory")
    check(digest.hexdigest() == EXPECTED,"all independent cost values")
    check(2130706433**6//2**128-TOTAL==134142152372895,"original reserve")
    print("PASS independent",blocks,"blocks",count,"boxes",mutations,"wrong floors")
    print("MAX",peak,"DIGEST",digest.hexdigest())
    print("Unrolled rank-profile products and both J corners; full tangent vectors retained")
    print("PASS every carrier on23000..23999; not an unrestricted row proof")
