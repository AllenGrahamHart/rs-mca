...
FIBER_TOTAL = 265879110627611677
TARGET = 274846585959022192
EXPECTED_DIGEST = "a71e75b1e32f0ec5715bc7e37b97d903e3cbd4b01543d21f661b9789e94e9a93"
...
def main():
    rows = [(a,min(a+15,HIGH)) for a in range(LOW,HIGH+1,16)]
    cover(rows)
    ...
    for J0,J1 in rows:
        ...
        for t in range(1,7):
            ell = 11-t
            lower,upper = (8001 if t==1 else ell),J1-t*(J1-4)//7-1
            f = ell*ell//4
            extra = [(D+f*(ell-1))//(f-1)+1]
            for k0,k1 in v.bins(lower,upper,128,extra):
    ...
    need(peak==(TARGET,(23000,23015,(6,3021,3046))),"exact maximum")
    need(len(rows)==63 and count==48357 and mutations==96714,"complete certificate inventory")
    need(digest.hexdigest()==EXPECTED_DIGEST,"every exact cost")
    need(2130706433**6//2**128-TARGET==134142152372895,"original reserve")
