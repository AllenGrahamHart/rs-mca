...
The [cumulative continuation](cumulative_low_weights.md) proves, for
s=11 and gap 67472, w(r;g)>=11*min(r,500). On one fixed selected
source this gives N<=floor(C/(11T)+(1/T)*sum_(t=1)^(T-1)L_t) for
1<=T<=500. These are nested raw-margin counts, not independent
support choices. It strengthens the SAME resource below and is used
by the finite consumer without an additional tuple or near budget.
