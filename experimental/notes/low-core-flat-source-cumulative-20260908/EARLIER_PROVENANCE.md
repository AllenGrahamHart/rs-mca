...
Historical provenance at ef8e3316; see [PROVENANCE.md](PROVENANCE.md) for
the extension. Its then-unpublished maximum-density argument is now included.
...
The pinned main already has evaluation-rank-flat routing and a
clone-tolerant basis-curve theorem (grande_finale.tex, lem:nonproper,
thm:clone-tolerant). These supply antecedents, not the claimed new finite
counts. Hughes's margin resource, raw-low core descent and gluing-rank-zero
payment in #1168/#1174/#1175 are likewise prerequisites or adjacent work.
The local registry's RACED warnings for rank routing are respected by
...
1. The occupied-subspace core-basis count beta(r), its combination by
   ...
   actual hypothesis from an arc and from mere projective fiber bounds.
2. Entire-source payment of bounded-flat and full progression carriers
   over the printed higher-J ranges, without a pair-curve-cover premise.
   The dimension-ten rational-multiplier classification already appeared
   in our weighted-cubic supplier; its whole-carrier payment is new here.
3. The raw-weighted extension through 9940, with actual doubled factor
   heights, complete cubic/reducible accounting and medium-margin bridge.
4. An all-multiplicity, all-positive-cutoff boundary for one sufficient
   degree-three full-kernel recipe on 9981..169999, not an unsafe source.

Greedy basis counting, Vandermonde interpolation, root bounds, scalar
extension, convexity and Hasse interpolation are classical methods. No
invention of those general methods or independent external review is claimed.

## Local Dependency Structure

~~~text
prior support-margin resource -> completed-basis resource
    + actual subspace occupancy -> core-basis/defect tuple count
    -> whole bounded-flat source payment
...
prior completed-basis raw-weight resource + cutoff-125 component estimates
    ...
    + all reducible patterns + medium-margin line bridge
    -> every-source interval 9822..9940
    + preceding interval -> every-source interval 4801..9940

prior multiplicity shell identity + convexity + large-r comparison
...
K3 / DIRECT / LOCAL_ONLY. To bank a row statement, bind the exact source
to an exhaustive original residual: actual rank/carrier, universal-core
cancellation, full-code-bad supports, all original finite labels and the
original near allowance. Active-v4 use additionally needs the first-match
ownership chronology and inherited charges. The generic theorem removes
maximality as a premise for its own count, not for older curve arguments.

No active U_Q, U_paid or U_new integer is supplied. Higher-J structural
conditions remain essential. Unrestricted endpoints, higher original
ranks, ordinary LIST and both prizes are unresolved. Publication changes
no local mathematical status, and maintainer acceptance remains external.
