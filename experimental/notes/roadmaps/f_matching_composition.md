...
## 5. Ledger correction 3: step (ii) is FALSE as stated at n = q-1

The DAG node's step (ii) claims the composed candidate always lands
in the finite `Dih_n` target (projective-multiplicative `X^e g(X^M)`
or dihedral). The proof attempt produced a counterexample, confirmed
on E35's own machinery (verifier CHECK 1).

**The q-flat.** In `F_17[X]`, `j = 4`, let `q = X^2 + 3X` and

```text
Q = span{ 1, q, q^2 }        (linear dim 3, projective dim 2).
```

Verified facts:

1. `Q` is stripped (`w1 = 0`) and carries `w2 = w2m = 7` minimal
   weight-2 supports, all disjoint (E35-profile `(2,2,2,2,2,2,2)`,
   listing size->=2 classes only): the pairs `{x, 14 - x}` inside
   `F_17^*` (fibers of `q`, since `q(x) = q(y), x != y` iff
   `x + y = -3`). The two remaining points of `H` are singleton
   classes: `x = 7` (the deck involution's fixed point) and `x = 14`
   (whose partner `0` is outside `H`) — within the `2e - 2 = 2`
   exceptional budget of Theorem 1(d).
2. `Q` is in NO `Dih_16` stratum: E35's full scan reports
   `mult_proj = dih = false` and ZERO anomaly flags — the flat is
   structurally invisible to detectors that only test elements of
   `Dih_16`.
3. `Q` is a span of three squarefree quartic divisors of `X^16 - 1`
   (root sets `{1,2,12,13}`, `{1,3,11,13}`, `{2,3,11,12}`, i.e.
   `(q-4)(q-10), (q-4)(q-1), (q-10)(q-1)`). So it lies in the ambient
   of E35's `b_djspan` family; the census (50 random triples out of
   ~10^9) simply never sampled it. The empty band 4-6 of the joint
   table is genuine, but `Q` sits at `w2m = 7` — INSIDE the
   "symmetric floor" band — nonsymmetric.
4. `Q` satisfies Theorem 1's conclusion perfectly: `e = 2`,
   `psi = q`, deck involution `iota(x) = -3 - x`, a Mobius involution
   in `PGL_2(F_17)` that does NOT normalize/stabilize `mu_16`. The
   composition step (i) is sound; what fails is confining `psi` to
   the `Dih_16` ledger.

**Sharp involution census at n = 16 (verifier CHECK 6).** Over
`F_17`, every Mobius involution pairs at most 8 points of `F_17^*`
into disjoint pairs; the value 8 is attained ONLY by the `Dih_16`
involutions (`x -> -x`, `x -> b/x`); exactly 160 non-`Dih_16`
involutions attain 7. Consequence: at `n = q - 1 = 16` the true
`Dih`-forcing threshold for involution-type flats is `w2m = 8`, not
`W* = 4`; the band `w2m = 7` admits non-`Dih` compositions.

**Scale diagnosis.** `n = q - 1` is DEGENERATE for step (ii): every
element of `PGL_2(F_q)` preserves `P^1(F_q)`, so `H = F_q^*` is
near-invariant under EVERY Mobius map and adaptedness of `psi` to `H`
is invisible. At a proper coset scale the same construction
collapses: on `H = mu_8 < F_17^*` the involution `x -> -3 - x` pairs
only `{1,13}` and `{15,16}` — `m = 2`, background level (verifier
CHECK 6). This is consistent with E36, which certifies that the SET
STABILIZER of the domain inside `PGL_2` is exactly dihedral — the
q-flat's `iota` does not stabilize `H`, it merely intersects it in
many 2-cycles, which is possible only at degenerate scale.

**Corrected step (ii) (OPEN — the named gap).** For `n <= (q-1)/2`:
if `m` exceeds the maximum number of disjoint psi-fiber pairs inside
`H` over all NON-`H`-adapted `psi` of degree `e <= j` — a
character-sum / Weil-type count, expected `~ n^2/q + O(sqrt(q))` for
`e = 2` — then `psi`'s fiber structure stabilizes `H`, and E36 /
`f_dih_subgroup_completeness` puts the candidate in the cyclic or
dihedral list. Missing pieces: (α) the character-sum lemma for
`#{x in H : iota(x) in H}` uniform over non-adapted Mobius `iota`;
(β) its extension to `e > 2` (fibers of non-adapted `psi` meeting `H`
twice); (γ) the regime statement `sqrt(q) << n <= (q-1)/2` where the
symmetric floor `~ n/2` separates from that count.

## 6. Label ledger and open items

```text
PROVED   Lemma B  (value rigidity, |V| <= 2(F-1)^2)         Sec 2
PROVED   Theorem 1 (i)-(c) composition at m > j + (j-1)^2   Sec 3
PROVED   Theorem 1 (d)  under char K > j (tameness)         Sec 3
PROVED   Lemma D  (dual-RS ratio law at m >= j + 2 - r)     Sec 4
FALSIFIED (as stated)  step (ii) "composition => Dih" at
         n = q - 1: the q-flat, w2m = 7, no Dih_16 stratum  Sec 5
OPEN     corrected step (ii): H-adaptedness via the
         character-sum count (gaps α, β, γ named)           Sec 5
OPEN     linear threshold W(d) = d + 2 for composition
         (route: Lemma D's law (D2) + Lemma B; today only
         the quadratic B(j) is proved)                      Sec 3-4
```

Remarks. (1) *K-rationality of psi*: Theorem 1 is stated over `Kbar`;
since `K = F_q` is perfect and is the exact constant field of both
`K(x)` and `K(R_2, ..., R_r)`, the constant-field extension preserves
degrees and Luroth over `K` provides a `K`-rational generator; none
of the counts above need this, so it is recorded as a remark, not
used. (2) The DAG statement of `f_weight2_inverse` must be repaired:
conclusion = psi-pullback membership (step (i) output, Theorem 1) +
a separate `H`-adaptedness clause with the corrected threshold; the
`W* = 4` calibration from E35 measures the ACCIDENTAL ceiling
(Lemma D's `j + 1 - d`), not the Dih-forcing threshold (which is 8 at
n = 16 by the involution census). (3) E35's census should gain a
`PGL_2-pullback` family (pullbacks of random non-adapted involutions)
— they populate exactly the band `w2m = 7` at `n = 16` that the
current sample attributes to symmetric flats alone.

## 7. Verifier contract

`experimental/scripts/verify_f_matching_composition.py` — stdlib
only, deterministic seeds, exact arithmetic in `F_17`, < 30 s:

```text
CHECK 1  q-flat certification: w1/w2/w2m/profile, no Dih_16
         stratum, zero anomaly flags, divisor-span identity,
         composition degree e = 2 (exact bivariate-gcd
         computation of e), fibers on H = orbits of x -> -3-x,
         and Theorem 1(b): every fiber pair is a flat-wide
         ratio edge.
CHECK 2  E35 census artifacts (all 67 dim-3 symmetric strata,
         j = 4, 5): every stripped flat has e >= 2; collision
         classes on H are unions of psi-fibers with generic
         size e; exceptional (short) classes <= 2e - 2
         (Theorem 1(d) budget); every in-class pair satisfies
         the flat-wide ratio identity (derived phi-form).
CHECK 3  near-pencils (E35 seeds): e = 1 for the ratio tuple
         (R_2 = X is Mobius — the exact clause they violate),
         every edge touches the cancellation locus Z(P_1), and
         m <= D + (D-1)^2.
CHECK 4  Lemma B sanity: seeded random generating pairs at
         degrees <= 5: rational V(T) count <= 2(F-1)^2 and
         e = 1 confirmed by gcd; a non-generating pair
         (functions of x^2): e = 2 and the y = -x component
         appears — the generation hypothesis is load-bearing.
CHECK 5  Lemma D exact: on the q-flat and on a census flat,
         build the dependency, reconstruct g by interpolation,
         verify deg g <= n - j - 2, the vanishing law (D1),
         the ratio law (D2), and |I| >= ceil((j+2)/2).
CHECK 6  involution census: non-Dih max disjoint pairs on
         F_17^* is 7 (160 attainers), Dih max is 8; mu_8
         collapse of the q-construction to m = 2.
CHECK 7  randomized exact checks at n = 16: seeded pullback
         flats (psi in {X^2, X^2+3X, X^2+5X, X^4}) all get
         e >= 2 with fiber-classes matching psi; seeded random
         flats get e = 1 and m <= 3 <= D + (D-1)^2.
```
