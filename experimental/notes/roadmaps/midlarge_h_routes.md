# Mid/large-h routes: the (~10, A] certification gap
...
- **Verifier:** `experimental/scripts/verify_c1b_descent_injection.py`
  (sections S10, S11, S12 plus the Part-1 gates; 19/19 PASS).
- **Companion:** `experimental/notes/roadmaps/c1b_descent_injection_lemma.md`
  (the `h <= ~10` lane; its feasibility re-pricing feeds section 1 here).
...
`(16,4,31^2)`), S12b (necessity of `p = -1`).  Current replay: **19 PASS,
