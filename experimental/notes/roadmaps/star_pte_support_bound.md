...
- **Verifier:** `experimental/scripts/verify_star_pte_support_bound.py`
  (5/5 PASS: the arithmetic identities, the corrected gap, and explicit
  finite-field constructions realising both range endpoints).
...
`r = k-1` giving the minimal trade `h = t+1`. Current run **5/5 PASS**.
