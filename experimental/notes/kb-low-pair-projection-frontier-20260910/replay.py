"""Replay the frozen six-node projection packet, serially and offline."""
...
MANIFEST_HASH = "01a2397889f1df87b3885fd66f192bd1af2236bfb4ea67ad7a747e1752c2a4fe"
PARENT = "3600bb59f57e087ad9243ff510d96dc21f77713c"
ROOTS = ["rate_half_mca_completed_high_raw_mass",
         "mca_moving_parameter_hyperplane_low_mass",
         "rate_half_mca_cubic_parameter_graph_payment",
         "pair_space_polynomial_projection_dichotomy",
         "rate_half_mca_moving_normal_basis_payment",
         "rate_half_mca_low_pair_projection_regular_frontier"]
SUPPLIERS = {
...
SCOPE = {
    ...
    "original_high44": {"J": [9941, 21499], "bound": 624373932788019251,
                        "upper_J": [14000, 21499], "upper_bound": 522680876725222604},
    "cubic_G8_bound": 271476187024062739,
    "cubic_G43_bound": 237527989066104848,
    "moving_normal_degrees": [2, 3, 4, 5],
    "moving_normal_G7_bound": 272944903448274111,
    "contained_lines_G7_bound": 272889015800964850,
    "deficient_projection_bound": 274138707278280353,
    "surviving_P7_ranks": list(range(16, 23)), "finite_exception_bound": 6,
}
...
def suppliers():
    ...
    for packet, pin in SUPPLIERS.items():
        ...
        for row in manifest["files"]:
            ...
            sources[key] = data
    need(len(sources) == 933, "published source inventory")
    return sources, graph


def validate(manifest, published, credited):
    need(manifest["schema"] == "kb-low-pair-projection-frontier-v1", "schema")
    ...
    need(manifest["inherited_proof_documents_compared"] == 273
         and manifest["published_source_hashes_checked"] == 933, "comparison inventory")
    files = {row["path"]: row for row in manifest["files"]}
    need(len(files) == len(manifest["files"]) == 51, "new source inventory")
    ...
    need(sum(map(len, sources.values())) == manifest["source_bytes"] == 81065, "new source size")
    documents = {}
    for row in manifest["inherited_sources"]:
        ...
        documents[key] = data
    need(len(documents) == 273, "inherited proof inventory")
    graph = manifest["requirements"]
    need(len(graph) == 96 and set(graph) == set(manifest["origin_node_manifest_sha256"]),
         "closure inventory")
...
def main():
    ...
    print("PASS 984 listed hashes; 273 inherited proof documents; 96-node acyclic closure;",
          mutations(manifest, published, credited), "malformed manifests rejected", flush=True)
    borrowed = {}
    for section, name in (("background", "rate_half_mca_low_raw_rank_filtration"),
    ...
    with tempfile.TemporaryDirectory(prefix="kb-pair-projection-") as temp:
        runtime = Path(temp)
    ...
    print("PASS eight new focused checks plus inherited filtration and raw-margin checks")
    print("Other inherited arithmetic not freshly replayed; independent mathematical review due")
    print("Regular projection, full degree interval, active atoms and both Prize problems remain open")
